...
class Verdict:
    ...
    def as_row(self):
        d = asdict(self)
        for k in ("majors", "companies", "countries"):
            d[k] = "; ".join(d[k])
...
def classify(title, year, meta) -> Verdict:
    ...
    note = "; ".join(notes) or "no budget text"
    if today is None:
        ...
        v.notes += note
        return v
    avg = "avg of 2 sources" if len(per_source) == 2 else v.budget_source
